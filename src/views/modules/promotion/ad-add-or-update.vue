...
<singleUpload v-model="dataForm.pic"></singleUpload>
    </el-form-item>
  ...
  methods: {
    init (id) {
      ...
      this.$nextTick(() => {
        ...
        if (this.dataForm.id) {
          this.$http({
            url: this.$http.adornUrl(`/promotion/ad/info/${this.dataForm.id}`),
            method: 'get',
            params: this.$http.adornParams()
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.dataForm.pic = data.ad.pic
              this.dataForm.url = data.ad.url
              this.dataForm.status = data.ad.status
            }
          })
        }
      })
        ...
        if (valid) {
          this.$http({
            url: this.$http.adornUrl(
              `/promotion/ad/${!this.dataForm.id ? 'save' : 'update'}`
            ),
            method: 'post',
